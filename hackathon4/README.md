...
| Fine-tunes or adapts an LLM | Adapts `Qwen/Qwen2.5-0.5B-Instruct` using rank-8 LoRA and Hugging Face PEFT; the completed run saves an adapter-only checkpoint. |
| Simple before/after comparison | Runs the unchanged base model and LoRA-adapted model on the same 50 held-out examples and displays both outputs side by side in Streamlit. |
...
The deployment model is [`Qwen/Qwen2.5-0.5B-Instruct`](https://huggingface.co/Qwen/Qwen2.5-0.5B-Instruct). It was selected because it is an instruction-following causal model with Transformers/PEFT support and is small enough for a stable hackathon deployment. An initial 1.5B experiment exceeded Streamlit Community Cloud memory, motivating this documented engineering tradeoff. The model code dynamically inspects supported projection layer names instead of assuming one architecture.
...
The completed run used 500 deterministic synthetic demonstration records from locally defined templates: 400 train, 50 validation, and 50 held-out test examples. Every record is marked `data_origin: synthetic_template`; `data/outputs/dataset_summary.json` states that the data are not clinically validated. No paid API, private record, or protected health information is required.
...
Adapted inference loads the same base model, attaches `models/medexplain_qwen05_lora_adapter/`, runs on the identical held-out IDs, and writes:
...
The following excerpt comes from held-out example `synthetic-0388-c5195ec3eb58` generated by the completed run:

**Input:** “MRI demonstrates a 1.5 cm enhancing lesion in the left temporal lobe. Findings are concerning for neoplasm.”

**Base-model excerpt:** “The doctor found out that there was a small bump on the brain, about the size of a pencil eraser ... they're worried it might be cancerous.”

**LoRA-adapted first response segment:** “The MRI shows a 1.5 cm area in the left temporal lobe that is showing up clearly on contrast. It could be a tumor, but more evaluation is needed to determine what it is.”

This example shows better preservation of the measurement, anatomy, and diagnostic uncertainty, but it is not a claim of general safety. Other held-out examples introduced unsupported details or lost information, and the negation heuristic worsened. Output stopping and medical fidelity remain material failure modes.
...
A completed prototype run trained for one epoch on 400 synthetic examples, produced a 4.1 MB PEFT adapter, and generated paired predictions for 50 held-out examples. BERTScore was intentionally skipped. These synthetic-data results are preliminary and cannot support clinical conclusions.

| Metric | Base model | LoRA-adapted model |
|---|---:|---:|
| ROUGE-1 F1 | 0.189 | 0.254 |
| ROUGE-2 F1 | 0.076 | 0.179 |
| ROUGE-L F1 | 0.147 | 0.224 |
| Flesch Reading Ease | 45.04 | 64.59 |
| Flesch-Kincaid grade | 11.80 | 8.55 |
| Missing-number heuristic flags | 17 / 50 | 4 / 50 |
| Missing-anatomy heuristic flags | 4 / 50 | 0 / 50 |
| New-diagnosis-language flags | 13 / 50 | 9 / 50 |
| New-treatment-language flags | 1 / 50 | 4 / 50 |
| Negation-change flags | 36 / 50 | 39 / 50 |

These results demonstrate that LoRA training and paired evaluation ran, not that the model is clinically ready. ROUGE and readability improved, and missing-number, missing-anatomy, and new-diagnosis flags decreased. New-treatment and negation-change flags worsened. Synthetic data, prompt-like continuations, heuristic limitations, and absent clinician review make the findings preliminary.
...
The deployed app defaults to **Live rewrite and comparison** using
`Qwen/Qwen2.5-0.5B-Instruct` with the trained LoRA adapter. It generates the unchanged
base-model output by temporarily disabling the adapter, then generates the adapted
output with the adapter enabled. The model resource is cached after its first load.

The secondary **Saved held-out comparisons** mode reads genuine paired predictions
produced by `python main.py evaluate`. It provides stable evaluation evidence without
regenerating outputs and trims only prompt-role spillover after the first response.

On macOS, an MPS tensor-size assertion may occur during Qwen generation. Force the
CPU backend for a reliable fallback by prefixing commands with
`MEDEXPLAIN_DEVICE=cpu`.
